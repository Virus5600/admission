...
(function ($) {
    ...
    $.fn.simpleCropper = function (onComplete) {
        ...
        var jcrop_api;
        var bottom_html = "<input type='file' id='fileInput' name='files[]'/ accept='image/*'><canvas id='myCanvas' style='display:none;'></canvas><div id='modal'></div><div id='preview'><div class='buttons'><div class='switch' title='Switch Aspect Ratio'></div><div class='cancel'></div><div class='ok'></div></div></div>";
        var aspectRatio = 1;                 // added by csn-05/17/2020
        var allowSwitchAspectRatio = false;  // added by csn-05/17/2020
        var imagetype = 'jpeg';              // added by csn-05/17/2020
        var imagequality = 0.92;             // added by csn-05/21/2020
        ...
        this.click(function () {
            ...
            // added by csn-05/17/2020
            // custom data options
            //     data-switch-aspect-ratio = 1 | true | yes | false    (default:false)
            //     data-image-type          = jpg | jpeg | png          (default:jpeg)
            allowSwitchAspectRatio = $(file_display_area).data('switchAspectRatio') ? true : false;
            imagetype = $(file_display_area).data('imageType');
            if ((imagetype === undefined) || (imagetype.match(/jpg|jpeg|png/g) == null))
                imagetype = 'jpeg';
            imagequality = $(file_display_area).data('imagequality'); 

            if ((imagequality === undefined) || isNaN(imagequality))
                imagequality = 0.92;
        ...
        $(document).ready(function () {
            ...
            // switch listener added by csn-05/17/2020
            $('.switch').click(function (event) {
                if (allowSwitchAspectRatio) {
                    if (aspX / aspY == aspectRatio)
                        aspectRatio = aspY / aspX;
                    else
        ...
        function imageUpload(dropbox) {
            ...
            if (file.type.match(imageType)) {
                ...
                reader.onload = function (e) {
                    ...
                    if ($(window).width() < 600) {
                        image_dimension_x = 250;
                        image_dimension_y = 250;
                    }
                    ...
                    current_image.onload = function () {
                        ...
                        aspectRatio = aspX / aspY;                                      // *
                        if ((aspectRatio != 1) && (allowSwitchAspectRatio == true))     // * added by csn-05/17/2020
                            $('.switch').show();                                        // * Enable switching aspect ratio
                        else                                                            // * for non-square image
                            $('.switch').hide();                                        // *
                        if (allowSwitchAspectRatio && (scaled_height < scaled_width))   // * 
                            aspectRatio = aspY / aspX;                                  // *
                        ...
                        ias = $(this).Jcrop({
                            ...
                            bgColor: '#747474',
                            bgOpacity: .4,
                            //aspectRatio: aspX / aspY,      // deleted by csn-05/17/2020
                            aspectRatio: aspectRatio,        // added   by csn-05/17/2020
        ...
        function preview() {
            ...
            if ((allowSwitchAspectRatio == 0) || (aspectRatio == aspX / aspY)) {   // added by csn-05/17/2020
                context.drawImage(current_image, (x1 * kw), (y1 * kh), (sw * kw), (sh * kh), 0, 0, aspX, aspY);
            }
            else {
                // *
                // * Added by csn-05/17/2020
                // * Reverse orientation of image if aspect ratio is reversed
                // * Just switch x and y and width and height
                // *

                // Set image original width and height
                var imgHeight = current_image.naturalWidth;
                var imgWidth = current_image.naturalHeight;

                // Set selection koeficient
                var kw = imgWidth / $("#preview").height();
                var kh = imgHeight / $("#preview").width();

                // Set canvas width and height and draw selection on it
                canvas.width = aspY;
                canvas.height = aspX;
                context.drawImage(current_image, (x1 * kw), (y1 * kh), (sw * kw), (sh * kh), 0, 0, aspY, aspX);
            }

            // Convert canvas image to normal img
            var dataUrl = canvas.toDataURL('image/' + imagetype, imagequality);
            ...
            if (onComplete) onComplete(
                {                    
                    ...
                    "canvas": {"width": canvas.width, "height" : canvas.height}          // added by csn-05/17/2020    
                },
                file_display_area // added by csn-05/17/2020 
               );
        }
