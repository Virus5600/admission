<?php
	...
	switch($_POST["form"]) {
		...
		case 'dsplaystudentadmissionlist':
			$res = mysqli_query($connection, "SELECT admission_id, CASE WHEN stud_Mname = '' OR stud_Mname IS NULL THEN CONCAT(stud_Lname, ', ', stud_Fname) ELSE CONCAT(stud_Lname, ', ', stud_Fname, ' ', LEFT(stud_Mname, '1'), '.') END, date_of_admission, stud_type FROM admission WHERE ((treasury_stat = '0' AND registrar_stat = '0') OR (treasury_stat = '1' AND registrar_stat = '0')) AND deny = '0'");
					...
					<li class="item itemhover" style="border-bottom: 1px solid #e9edf0;">
		                <div class="item-row">
		                    ...
		                    <div class="item-col item-col-sales no-overflow">
		                        <div> 
		                        	<a href="#">
		                            	<h4 class="item-title no-wrap" title="<?php echo $row[0]; ?>"><?php echo $row[0]; ?></h4> 
		                        	</a>
		                        </div>
		                    </div>
		...
		case 'dsplayschedulelist':
			$res = mysqli_query($connection, "SELECT stud_no, CASE WHEN stud_Mname = '' OR stud_Mname IS NULL THEN CONCAT(stud_Lname, ', ', stud_Fname) ELSE CONCAT(stud_Lname, ', ', stud_Fname, ' ', LEFT(stud_Mname, '1'), '.') END, date_of_admission, stud_type FROM admission WHERE registrar_stat = '1' AND treasury_stat = '1' AND sched_stat = '0'");
			$countstud = mysqli_num_rows($res);
			if($countstud == TRUE){
				while($row = mysqli_fetch_array($res)){
					if($row[3] == 0){
